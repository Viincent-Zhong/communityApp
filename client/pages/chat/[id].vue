  ...
  <div class="w-full mt-24">
    <div class="ml-24 w-[70%] bg-slate-200 p-4">
      ...
      <div class="w-full mt-10">
          <h1 class="w-full text-center text-5xl underline font-mono text-zinc-800">{{ chatRoom.title }}</h1>
          <p class="mt-10 ml-10 text-2xl text-zinc-800">{{ chatRoom.content }}</p>
        </div>
...
const { chatRoom, getChatRoom } = useChatRoom();
const { comments, createComment, getComments } = useComment();

const handleCreateComment = async () => {
  await createComment(comment.value, chatRoom.value.id, CommentType.Chat);
  comment.value = '';
  window.location.reload();
}

onMounted(async () => {
  await getChatRoom(id);
  await getComments(chatRoom.value.id, CommentType.Chat);
});
</script>