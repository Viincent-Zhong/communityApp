  ...
  <div class="w-full mt-24">
    <div class="ml-24 w-[70%] bg-slate-200 p-4">
      ...
      <div class="w-full mt-10">
          ...
          <h1 class="mt-6 w-full text-2xl underline font-mono text-zinc-800">Scheduled for {{ sellRoom.date }}</h1>
          <h1 class="mt-2 w-full text-center text-5xl font-mono text-zinc-800">{{ sellRoom.title }}</h1>
          <p class="mt-10 text-2xl text-zinc-800">{{ sellRoom.content }}</p>
      </div>
      <!-- Border -->
...
const { sellRoom, getSellRoom } = useSellRoom();
const { comments, createComment, getComments} = useComment();

const handleCreateComment = async () => {
  await createComment(comment.value, sellRoom.value.id, CommentType.Sells);
  comment.value = '';
  window.location.reload();
}

onMounted(async () => {
  await getSellRoom(id);
  await getComments(sellRoom.value.id, CommentType.Sells);
});
</script>